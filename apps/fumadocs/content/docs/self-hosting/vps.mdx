...
description: Deploy with Docker Compose on any VPS
---
...
# Allow Redis (only if control plane is external)
...
Make Docker Compose start on boot:

```bash
sudo tee /etc/systemd/system/rediredge.service > /dev/null <<EOF
[Unit]
Description=Rediredge Data Plane
...
Type=oneshot
RemainAfterExit=yes
WorkingDirectory=/home/$USER/rediredge/deploy
...
sudo systemctl enable rediredge
sudo systemctl start rediredge
```
...
- [Configure control plane](https://github.com/leonardotrapani/rediredge#control-plane)
